...
VALUES ('$name')";
    ...
    public function subcategorys($category, $subcategory)
    {
        ...
        if ($query) {
            $this->session->set_flashdata('addsub_success', 'Sub Category added successfully!');
            redirect('sub_admin/Category/subcategory');
        } else {
            $this->session->set_flashdata('addsub_error', 'Something went wrong. Please try again!');
            redirect('sub_admin/Category/subcategory');
        }
    }
